...
| `park_multi_proof.py` | One VM with **four concurrent** in-flight upstream requests is parked and revived — all four resume. |
...
| `wallclock_proof.py` | Fresh boot has a real wall clock (year ≥ 2026, not 1970 — PL031 RTC); across a park the wall clock freezes while monotonic stays continuous, and one `hwclock -s` reconciles it. |
