...
/// Build hvm_start_info (version 1) plus its memmap table and the command line
/// into `buf` at guest base `base`, describing `layout` RAM and pointing the
/// kernel at `rsdp`. Returns the start_info physical address and bytes used.
pub fn buildStartInfo(buf: []u8, base: u64, layout: memmap.Layout, rsdp: u64, cmdline: []const u8) StartInfo {
    @memset(buf, 0);
    var pos: usize = 0;
    ...
    w32(buf, si_off + 12, 0); // nr_modules
    w64(buf, si_off + 16, 0); // modlist_paddr
...
/// Load `kernel`, place ACPI + start_info in guest RAM, and set `vcpu` to enter
/// the kernel via the PVH protocol.
pub fn boot(vm: *vmm.Vm, vcpu: *vmm.Vcpu, layout: memmap.Layout, kernel: []const u8, cmdline: []const u8) !void {
    const Ctx = struct {
        vm: *vmm.Vm,
    ...
    const si = buildStartInfo(&si_buf, start_info_addr, layout, tables.rsdp_addr, cmdline);
...
test "start_info has the PVH magic and links its tables" {
    ...
    const si = buildStartInfo(&buf, 0x2000, layout, 0x7abc, "console=ttyS0");
