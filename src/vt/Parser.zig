...
//!
//! VENDORED from ghostty (src/terminal/Parser.zig), ported to Zig 0.16. The
//! parsing logic is verbatim; the debug `format` methods and the valgrind init
//! block were removed, and the OSC sub-parser was swapped for Nether's minimal
//! zero-alloc osc.zig. See PORTING.md for provenance and the exact changes.
const Parser = @This();
    ...
    return switch (action) {
        ...
        .csi_dispatch => csi_dispatch: {
            ...
            if (c != 'm' and self.params_sep.count() > 0) {
                @branchHint(.cold);
                log.warn(
                    "CSI colon or mixed separators only allowed for 'm' command, got final {c}",
                    .{c},
                );
...
// Note: ghostty's OSC color-operation tests (osc 112/104) were dropped here;
// Nether's minimal osc.zig does not interpret color operations. See PORTING.md.
